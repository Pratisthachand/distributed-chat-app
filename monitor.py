...
class ChatMonitor:
    ...
    def receive_messages(self, server_id, sock):
        """Receive messages from a server using proper line buffering"""
        # Create a line buffer for proper TCP stream handling
        buffer = b""
        try:
            while True:
            ...
            pass  # Connection lost, silently exit thread
        ...
        with self.lock:
            ...
            if msg.startswith("[T:"):
                ...
                if timestamp_end > 0:
                    ...
                    content = msg[timestamp_end+2:].strip()        # Rest of message
                    ...
                    if timestamp_num is not None:
                        ...
                        if timestamp_num not in self.seen_timestamps:
                            ...
                            if not skip_system:
                                self.display_message(timestamp_str, content, msg)
    
    def display_message(self, timestamp, content, full_msg):
